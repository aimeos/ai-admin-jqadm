...
namespace Aimeos\Admin\JQAdm\Type\Customer\Lists;

sprintf( 'type/customer/lists' ); // for translation


/**
 * Default implementation of customer list type JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	public function copy()
	{
		...
		try
		{
			...
			$manager = \Aimeos\MShop\Factory::createManager( $context, 'customer/lists/type' );
		...
		{
			$error = array( 'type-customer-lists-item' => $context->getI18n()->dt( 'mshop', $e->getMessage() ) );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
		catch( \Exception $e )
		{
			$error = array( 'type-customer-lists-item' => $e->getMessage() . ', ' . $e->getFile() . ':' . $e->getLine() );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
	...
	public function create()
	{
		...
		try
		{
			...
			if( !isset( $view->item ) ) {
				$view->item = \Aimeos\MShop\Factory::createManager( $context, 'customer/lists/type' )->createItem();
			}

			$data['customer.lists.type.siteid'] = $view->item->getSiteId();
	...
	public function delete()
	{
		...
		try
		{
			...
			$this->nextAction( $view, 'search', 'type/customer/lists', null, 'delete' );
			return;
		}
		catch( \Aimeos\MShop\Exception $e )
		{
	...
	public function save()
	{
		...
		try
		{
			...
			$this->nextAction( $view, $view->param( 'next' ), 'type/customer/lists', $view->item->getId(), 'save' );
			return;
		}
	...
	public function search()
	{
		...
		{
			$total = 0;
			$params = $this->storeSearchParams( $view->param(), 'type/customer/lists' );
			$manager = \Aimeos\MShop\Factory::createManager( $context, 'customer/lists/type' );
		...
		/** admin/jqadm/type/customer/lists/template-list
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/type/customer/lists/template-list';
		$default = 'type/customer/lists/list-standard.php';
	...
	public function getSubClient( $liststype, $name = null )
	{
		 ...
		 * @see admin/jqadm/type/customer/lists/decorators/local
		 */
	...
	{
		/** admin/jqadm/type/customer/lists/standard/subparts
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/type/customer/lists/standard/subparts', [] );
	}
	...
	protected function fromArray( array $data )
	{
		...
		if( isset( $data['customer.lists.type.id'] ) && $data['customer.lists.type.id'] != '' ) {
			$item = $manager->getItem( $data['customer.lists.type.id'] );
		} else {
	...
	protected function toArray( \Aimeos\MShop\Common\Item\Type\Iface $item, $copy = false )
	{
		...
		if( $copy === true )
		{
			$data['customer.lists.type.code'] = $data['customer.lists.type.code'] . '_copy';
			$data['customer.lists.type.id'] = '';
		}
	...
	{
		/** admin/jqadm/type/customer/lists/template-item
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/type/customer/lists/template-item';
		$default = 'type/customer/lists/item-standard.php';
