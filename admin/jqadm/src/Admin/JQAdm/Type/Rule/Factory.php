...
/**
 * Factory for rule type JQAdm client
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/**
	 * Creates a rule type client object
	 *
	...
	public static function create( \Aimeos\MShop\Context\Item\Iface $context, string $name = null ) : \Aimeos\Admin\JQAdm\Iface
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'admin/jqadm/type/rule/name', 'Standard' );
		}

		$iface = '\\Aimeos\\Admin\\JQAdm\\Iface';
		$classname = '\\Aimeos\\Admin\\JQAdm\\Type\\Rule\\' . $name;
		...
		return self::addClientDecorators( $context, $client, 'type/rule' );
	}
