...
sprintf( 'type/price' ); // for translation


/**
 * Default implementation of type price JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	public function copy()
	{
		...
		{
			$error = array( 'type-price-item' => $context->getI18n()->dt( 'mshop', $e->getMessage() ) );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
		catch( \Exception $e )
		{
			$error = array( 'type-price-item' => $e->getMessage() . ', ' . $e->getFile() . ':' . $e->getLine() );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
	...
	public function create()
	{
		...
		try
		{
			...
			if( !isset( $view->item ) ) {
				$view->item = \Aimeos\MShop\Factory::createManager( $context, 'price/type' )->createItem();
			}

			$data['price.type.siteid'] = $view->item->getSiteId();
	...
	public function delete()
	{
		...
		try
		{
			...
			$this->nextAction( $view, 'search', 'type/price', null, 'delete' );
			return;
		}
		catch( \Aimeos\MShop\Exception $e )
		{
	...
	public function save()
	{
		...
		try
		{
			...
			$this->nextAction( $view, $view->param( 'next' ), 'type/price', $view->item->getId(), 'save' );
			return;
		}
	...
	public function search()
	{
		...
		{
			$total = 0;
			$params = $this->storeSearchParams( $view->param(), 'type/price' );
			$manager = \Aimeos\MShop\Factory::createManager( $context, 'price/type' );
		...
		/** admin/jqadm/type/price/template-list
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/type/price/template-list';
		$default = 'type/price/list-standard.php';
	...
	public function getSubClient( $type, $name = null )
	{
		 ...
		 * @see admin/jqadm/type/price/decorators/global
		 * @see admin/jqadm/type/price/decorators/local
		 */
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/type/price/decorators/excludes
		 * @see admin/jqadm/type/price/decorators/local
		 */
		 ...
		 * @see admin/jqadm/type/price/decorators/global
		 */
		return $this->createSubClient( 'type/price' . $type, $name );
	}
	...
	{
		/** admin/jqadm/type/price/standard/subparts
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/type/price/standard/subparts', [] );
	}
	...
	protected function fromArray( array $data )
	{
		...
		if( isset( $data['price.type.id'] ) && $data['price.type.id'] != '' ) {
			$item = $manager->getItem( $data['price.type.id'] );
		} else {
	...
	protected function toArray( \Aimeos\MShop\Common\Item\Type\Iface $item, $copy = false )
	{
		...
		if( $copy === true )
		{
			$data['price.type.code'] = $data['price.type.code'] . '_copy';
			$data['price.type.id'] = '';
		}
	...
	{
		/** admin/jqadm/type/price/template-item
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/type/price/template-item';
		$default = 'type/price/item-standard.php';
