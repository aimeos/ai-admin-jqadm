...
/**
 * Factory for price lists type JQAdm client
 *
 * @package Admin
 * @subpackage JQAdm
 */
	...
	implements \Aimeos\Admin\JQAdm\Common\Factory\Iface
{
	...
	public static function createClient( \Aimeos\MShop\Context\Item\Iface $context, array $templatePaths, $name = null )
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'admin/jqadm/type/price/lists/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Admin\\JQAdm\\Type\\Price\\Lists\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Admin\\JQAdm\\Type\\Price\\Lists\\' . $name;

		$client = self::createClientBase( $context, $classname, $iface, $templatePaths );

		return self::addClientDecorators( $context, $client, $templatePaths, 'type/price/lists' );
	}
