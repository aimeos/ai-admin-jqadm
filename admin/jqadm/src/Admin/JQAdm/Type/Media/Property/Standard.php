...
sprintf( 'type/media/property' ); // for translation


/**
 * Default implementation of media list type JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	public function copy()
	{
		...
		try
		{
			...
			$manager = \Aimeos\MShop\Factory::createManager( $context, 'media/property/type' );
		...
		{
			$error = array( 'type-media-property-item' => $context->getI18n()->dt( 'mshop', $e->getMessage() ) );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
		catch( \Exception $e )
		{
			$error = array( 'type-media-property-item' => $e->getMessage() . ', ' . $e->getFile() . ':' . $e->getLine() );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
	...
	public function create()
	{
		...
		try
		{
			...
			if( !isset( $view->item ) ) {
				$view->item = \Aimeos\MShop\Factory::createManager( $context, 'media/property/type' )->createItem();
			}

			$data['media.property.type.siteid'] = $view->item->getSiteId();
	...
	public function delete()
	{
		...
		try
		{
			...
			$this->nextAction( $view, 'search', 'type/media/property', null, 'delete' );
			return;
		}
		catch( \Aimeos\MShop\Exception $e )
		{
	...
	public function save()
	{
		...
		try
		{
			...
			$this->nextAction( $view, $view->param( 'next' ), 'type/media/property', $view->item->getId(), 'save' );
			return;
		}
	...
	public function search()
	{
		...
		{
			$total = 0;
			$params = $this->storeSearchParams( $view->param(), 'type/media/property' );
			$manager = \Aimeos\MShop\Factory::createManager( $context, 'media/property/type' );
		...
		/** admin/jqadm/type/media/property/template-list
		...
		$default = 'type/media/property/list-standard.php';
	...
	public function getSubClient( $propertytype, $name = null )
	{
		 ...
		 * @see admin/jqadm/type/media/property/decorators/local
		 */
	...
	{
		/** admin/jqadm/type/media/property/standard/subparts
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/type/media/property/standard/subparts', [] );
	}
	...
	protected function fromArray( array $data )
	{
		...
		if( isset( $data['media.property.type.id'] ) && $data['media.property.type.id'] != '' ) {
			$item = $manager->getItem( $data['media.property.type.id'] );
		} else {
	...
	protected function toArray( \Aimeos\MShop\Common\Item\Type\Iface $item, $copy = false )
	{
		...
		if( $copy === true )
		{
			$data['media.property.type.code'] = $data['media.property.type.code'] . '_copy';
			$data['media.property.type.id'] = '';
		}
	...
	{
		/** admin/jqadm/type/media/property/template-item
		...
		$default = 'type/media/property/item-standard.php';
