	...
	implements \Aimeos\Admin\JQAdm\Common\Factory\Iface
{
	...
	{
		/** admin/jqadm/type/attribute/lists/name
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'admin/jqadm/type/attribute/lists/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Admin\\JQAdm\\Type\\Attribute\\Lists\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Admin\\JQAdm\\Type\\Attribute\\Lists\\' . $name;

		$client = self::createClientBase( $context, $classname, $iface, $templatePaths );

		return self::addClientDecorators( $context, $client, $templatePaths, 'type/attribute/lists' );
	}
