	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	public function copy()
	{
		...
		{
			$error = array( 'type-stock-item' => $context->getI18n()->dt( 'mshop', $e->getMessage() ) );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
		catch( \Exception $e )
		{
			$error = array( 'type-stock-item' => $e->getMessage() . ', ' . $e->getFile() . ':' . $e->getLine() );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
	...
	public function create()
	{
		...
		try
		{
			...
			if( !isset( $view->item ) ) {
				$view->item = \Aimeos\MShop::create( $context, 'stock/type' )->createItem();
			}

			$data['stock.type.siteid'] = $view->item->getSiteId();
	...
	public function delete()
	{
		...
		try
		{
			...
			$search->setConditions( $search->compare( '==', 'stock.type.id', $ids ) );
			...
			$this->nextAction( $view, 'search', 'type/stock', null, 'delete' );
			return;
		}
		catch( \Aimeos\MShop\Exception $e )
		{
	...
	public function save()
	{
		...
		try
		{
			...
			$this->nextAction( $view, $view->param( 'next' ), 'type/stock', $view->item->getId(), 'save' );
			return;
		}
	...
	public function search()
	{
		...
		{
			$total = 0;
			$params = $this->storeSearchParams( $view->param(), 'type/stock' );
			$manager = \Aimeos\MShop::create( $context, 'stock/type' );
	...
	{
		/** admin/jqadm/type/stock/standard/subparts
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/type/stock/standard/subparts', [] );
	}
	...
	protected function fromArray( array $data )
	{
		...
		if( isset( $data['stock.type.id'] ) && $data['stock.type.id'] != '' ) {
			$item = $manager->getItem( $data['stock.type.id'] );
		} else {
	...
	protected function toArray( \Aimeos\MShop\Common\Item\Type\Iface $item, $copy = false )
	{
		...
		if( $copy === true )
		{
			$data['stock.type.code'] = $data['stock.type.code'] . '_copy';
			$data['stock.type.id'] = '';
		}
