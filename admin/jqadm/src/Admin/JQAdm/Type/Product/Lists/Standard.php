...
namespace Aimeos\Admin\JQAdm\Type\Product\Lists;

sprintf( 'type/product/lists' ); // for translation
	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	public function copy()
	{
		...
		{
			$error = array( 'type-product-lists-item' => $context->getI18n()->dt( 'mshop', $e->getMessage() ) );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
		catch( \Exception $e )
		{
			$error = array( 'type-product-lists-item' => $e->getMessage() . ', ' . $e->getFile() . ':' . $e->getLine() );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
	...
	public function create()
	{
		...
		try
		{
			...
			if( !isset( $view->item ) ) {
				$view->item = \Aimeos\MShop::create( $context, 'product/lists/type' )->createItem();
			}

			$data['product.lists.type.siteid'] = $view->item->getSiteId();
	...
	public function delete()
	{
		...
		try
		{
			...
			$search->setConditions( $search->compare( '==', 'product.lists.type.id', $ids ) );
			...
			$this->nextAction( $view, 'search', 'type/product/lists', null, 'delete' );
			return;
		}
		catch( \Aimeos\MShop\Exception $e )
		{
	...
	public function save()
	{
		...
		try
		{
			...
			$this->nextAction( $view, $view->param( 'next' ), 'type/product/lists', $view->item->getId(), 'save' );
			return;
		}
	...
	public function search()
	{
		...
		{
			$total = 0;
			$params = $this->storeSearchParams( $view->param(), 'type/product/lists' );
			$manager = \Aimeos\MShop::create( $context, 'product/lists/type' );
		...
		/** admin/jqadm/type/product/lists/template-list
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/type/product/lists/template-list';
	...
	public function getSubClient( $liststype, $name = null )
	{
		 ...
		 * @see admin/jqadm/type/product/lists/decorators/global
		 * @see admin/jqadm/type/product/lists/decorators/local
		 */
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/type/product/lists/decorators/excludes
		 * @see admin/jqadm/type/product/lists/decorators/local
		 */
		 ...
		 * @see admin/jqadm/type/product/lists/decorators/global
		 */
		return $this->createSubClient( 'type/product/lists' . $liststype, $name );
	}
	...
	{
		/** admin/jqadm/type/product/lists/standard/subparts
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/type/product/lists/standard/subparts', [] );
	}
	...
	{
		$manager = \Aimeos\MShop::create( $this->getContext(), 'product/lists/type' );

		if( isset( $data['product.lists.type.id'] ) && $data['product.lists.type.id'] != '' ) {
			$item = $manager->getItem( $data['product.lists.type.id'] );
		} else {
	...
	protected function toArray( \Aimeos\MShop\Common\Item\Type\Iface $item, $copy = false )
	{
		...
		if( $copy === true )
		{
			$data['product.lists.type.code'] = $data['product.lists.type.code'] . '_copy';
			$data['product.lists.type.id'] = '';
		}
	...
	{
		/** admin/jqadm/type/product/lists/template-item
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/type/product/lists/template-item';
