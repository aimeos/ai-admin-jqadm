...
sprintf( 'type/plugin' ); // for translation


/**
 * Default implementation of type plugin JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	public function copy()
	{
		...
		try
		{
			...
			$manager = \Aimeos\MShop\Factory::createManager( $context, 'plugin/type' );
		...
		{
			$error = array( 'type-plugin-item' => $context->getI18n()->dt( 'mshop', $e->getMessage() ) );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
		catch( \Exception $e )
		{
			$error = array( 'type-plugin-item' => $e->getMessage() . ', ' . $e->getFile() . ':' . $e->getLine() );
			$view->errors = $view->get( 'errors', [] ) + $error;
			$this->logException( $e );
		}
	...
	public function delete()
	{
		...
		try
		{
			...
			$this->nextAction( $view, 'search', 'type/plugin', null, 'delete' );
			return;
		}
		catch( \Aimeos\MShop\Exception $e )
		{
	...
	public function save()
	{
		...
		try
		{
			...
			$this->nextAction( $view, $view->param( 'next' ), 'type/plugin', $view->item->getId(), 'save' );
			return;
		}
	...
	public function search()
	{
		...
		{
			$total = 0;
			$params = $this->storeSearchParams( $view->param(), 'type/plugin' );
			$manager = \Aimeos\MShop\Factory::createManager( $context, 'plugin/type' );
		...
		/** admin/jqadm/type/plugin/template-list
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/type/plugin/template-list';
		$default = 'type/plugin/list-standard.php';
	...
	public function getSubClient( $type, $name = null )
	{
		 ...
		 * @see admin/jqadm/type/plugin/decorators/global
		 * @see admin/jqadm/type/plugin/decorators/local
		 */
		 ...
		 * @category Developer
		 * @see admin/jqadm/common/decorators/default
		 * @see admin/jqadm/type/plugin/decorators/excludes
		 * @see admin/jqadm/type/plugin/decorators/local
		 */
		 ...
		 * @see admin/jqadm/type/plugin/decorators/global
		 */
		return $this->createSubClient( 'type/plugin' . $type, $name );
	}
	...
	{
		/** admin/jqadm/type/plugin/standard/subparts
		 ...
		 * @category Developer
		 */
		return $this->getContext()->getConfig()->get( 'admin/jqadm/type/plugin/standard/subparts', [] );
	}
	...
	{
		$manager = \Aimeos\MShop\Factory::createManager( $this->getContext(), 'plugin/type' );

		if( isset( $data['plugin.type.id'] ) && $data['plugin.type.id'] != '' ) {
			$item = $manager->getItem( $data['plugin.type.id'] );
		} else {
	...
	{
		/** admin/jqadm/type/plugin/template-item
		 ...
		 * @category Developer
		 */
		$tplconf = 'admin/jqadm/type/plugin/template-item';
		$default = 'type/plugin/item-standard.php';
