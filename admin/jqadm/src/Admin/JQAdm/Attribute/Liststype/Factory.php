	...
	implements \Aimeos\Admin\JQAdm\Common\Factory\Iface
{
	...
	{
		/** admin/jqadm/attribute/liststype/name
		 ...
		 *
		 *  admin/jqadm/attribute/liststype/name = Myfavorite
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'admin/jqadm/attribute/liststype/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Admin\\JQAdm\\Attribute\\Liststype\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Admin\\JQAdm\\Attribute\\Liststype\\' . $name;

		$client = self::createClientBase( $context, $classname, $iface, $templatePaths );

		return self::addClientDecorators( $context, $client, $templatePaths, 'attribute/liststype' );
	}
