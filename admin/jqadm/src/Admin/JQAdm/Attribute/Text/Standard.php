	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	protected function fromArray( \Aimeos\MShop\Attribute\Item\Iface $item, array $data )
	{
		...
		foreach( $data as $idx => $entry )
		{
			...
			if( ( $listItem = $item->getListItem( 'text', $entry['attribute.lists.type'], $entry['text.id'] ) ) === null ) {
	...
	protected function toArray( \Aimeos\MShop\Attribute\Item\Iface $item, $copy = false )
	{
		...
		foreach( $item->getListItems( 'text', null, null, false ) as $id => $listItem )
		{
			if( ( $refItem = $listItem->getRefItem() ) === null ) {
				continue;
			}
	...
	protected function render( \Aimeos\MW\View\Iface $view )
	{
		...
		$default = 'attribute/item-text-standard.php';
