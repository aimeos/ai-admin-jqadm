...
/**
 * Factory for settings JQAdm client
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/**
	 * Creates a settings client object
	 *
	...
	public static function create( \Aimeos\MShop\Context\Item\Iface $context, string $name = null ) : \Aimeos\Admin\JQAdm\Iface
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'admin/jqadm/settings/name', 'Standard' );
		}

		$iface = '\\Aimeos\\Admin\\JQAdm\\Iface';
		$classname = '\\Aimeos\\Admin\\JQAdm\\Settings\\' . $name;
		...
		return self::addClientDecorators( $context, $client, 'settings' );
	}
