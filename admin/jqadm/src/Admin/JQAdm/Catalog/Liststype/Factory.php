...
/**
 * Factory for catalog lists type JQAdm client
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/**
	 * Creates a catalog lists type client object
	 *
	...
	{
		/** admin/jqadm/catalog/liststype/name
		 ...
		 *
		 *  admin/jqadm/catalog/liststype/name = Myfavorite
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'admin/jqadm/catalog/liststype/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Admin\\JQAdm\\Catalog\\Liststype\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Admin\\JQAdm\\Catalog\\Liststype\\' . $name;

		$client = self::createClientBase( $context, $classname, $iface, $templatePaths );

		return self::addClientDecorators( $context, $client, $templatePaths, 'catalog/liststype' );
	}
