...
/**
 * Default implementation of catalog image JQAdm client.
 *
 * @package Admin
 * @subpackage JQAdm
 */
...
{
	/** admin/jqadm/catalog/image/name
	 * Name of the image subpart used by the JQAdm catalog implementation
	 *
	 * Use "Myname" if your class is named "\Aimeos\Admin\Jqadm\Catalog\Image\Myname".
	...
	public function getSubClient( $type, $name = null )
	{
		 ...
		 *
		 *  admin/jqadm/catalog/image/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see admin/jqadm/catalog/image/decorators/local
		 */

		/** admin/jqadm/catalog/image/decorators/global
		 ...
		 *
		 *  admin/jqadm/catalog/image/decorators/global = array( 'decorator1' )
		 *
		...
		/** admin/jqadm/catalog/image/decorators/local
		 ...
		 *
		 *  admin/jqadm/catalog/image/decorators/local = array( 'decorator2' )
		 *
	...
	protected function cleanupItems( array $listItems, array $listIds )
	{
		...
		$expr = array(
			$search->compare( '==', 'catalog.lists.refid', array_keys( $rmItems ) ),
			$search->compare( '==', 'catalog.lists.domain', 'media' ),
			$search->compare( '==', 'catalog.lists.type.code', 'default' ),
			$search->compare( '==', 'catalog.lists.type.domain', 'media' ),
		);
		$search->setConditions( $search->combine( '&&', $expr ) );
		$search->setSlice( 0, 0x7fffffff );

		foreach( $listManager->aggregate( $search, 'catalog.lists.refid' ) as $key => $count )
		{
			if( $count > 1 ) {
				unset( $rmItems[$key] );
			} else {
	...
	protected function createItem()
	{
		...
		$item->setTypeId( $typeManager->findItem( 'default', [], 'catalog' )->getId() );
		$item->setDomain( 'catalog' );
		$item->setStatus( 1 );
	...
	/**
	 * Returns the available catalog list types for media references
	 *
	 * @return \Aimeos\MShop\Common\Item\Type\Iface[] Associative list of catalog list type ID as keys and items as values
	 */
	protected function getMediaListTypes()
	{
		...
		$search->setSortations( array( $search->sort( '+', 'catalog.lists.type.label' ) ) );
	...
	{
		/** admin/jqadm/catalog/image/standard/subparts
		 * List of JQAdm sub-clients rendered within the catalog image section
		 *
