	...
	<div id="item-image-group" role="tablist" aria-multiselectable="true"
		...
		<div v-for="(id, idx) in items['catalog.lists.id']" class="group-item card">
			...
			<div v-bind:id="'item-image-group-data-' + idx" v-bind:class="getCss(idx)"
						...
						<div class="form-group row mandatory">
							...
							<div class="col-sm-8">
								<select class="form-control custom-select listitem-typeid" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
									name="<?= $enc->attr( $this->formparam( array( 'image', 'catalog.lists.typeid', '' ) ) ); ?>"
									v-bind:readonly="checkSite('catalog.lists.siteid', idx)"
									v-model="items['catalog.lists.typeid'][idx]" >

									<?php foreach( $this->get( 'imageListTypes', [] ) as $id => $typeItem ) : ?>
										<option value="<?= $enc->attr( $id ); ?>" v-bind:selected="items['catalog.lists.typeid'][idx] == '<?= $enc->attr( $id ) ?>'" >
											<?= $enc->html( $typeItem->getLabel() ); ?>
										</option>
									<?php endforeach; ?>
								</select>
					...
					<div class="form-group row optional">
						...
						<div class="col-sm-8">
							<input class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
								name="<?= $enc->attr( $this->formparam( array( 'image', 'catalog.lists.datestart', '' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:readonly="checkSite('catalog.lists.siteid', idx)"
								v-model="items['catalog.lists.datestart'][idx]" />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
					...
					<div class="form-group row optional">
						...
						<div class="col-sm-8">
							<input class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
								name="<?= $enc->attr( $this->formparam( array( 'image', 'catalog.lists.dateend', '' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:readonly="checkSite('catalog.lists.siteid', idx)"
								v-model="items['catalog.lists.dateend'][idx]" />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
				...
				<div v-show="advanced[idx]" class="col-xl-6 content-block secondary" v-bind:class="items['catalog.lists.siteid'][idx] != '<?= $this->site()->siteid() ?>' ? 'readonly' : ''">
