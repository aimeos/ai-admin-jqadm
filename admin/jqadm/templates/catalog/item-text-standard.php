			...
			<div is="draggable" v-model="items" group="text" handle=".act-move">
				<div v-for="(entry, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-text-group-data-' + idx" v-bind:class="getCss(idx)"
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" class="form-control custom-select listitem-type" required
											...
											v-bind:tabindex="'<?= $this->get( 'tabindex' ); ?>'"
											v-bind:value="entry['catalog.lists.type'] || 'default'" >
										</select>
									</div>
									<div class="col-sm-12 form-text text-muted help-text">
						...
						<config-table inline-template v-bind:idx="idx" v-bind:items="getConfig(idx)" v-bind:readonly="checkSite('catalog.lists.siteid', idx)">
