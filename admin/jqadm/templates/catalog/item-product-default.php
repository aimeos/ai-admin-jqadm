 ...
 * @category Developer
 */
$default = ['catalog.lists.status', 'catalog.lists.typeid', 'catalog.lists.config', 'product.label'];
			...
			<?php foreach( $listItems as $listId => $listItem ) : ?>
				<tr class="<?= $this->site()->readonly( $listItem->getSiteId() ); ?>">
					<?php if( in_array( 'catalog.lists.status', $fields ) ) : ?>
						<td class="catalog-lists-status"><div class="fa status-<?= $enc->attr( $listItem->getStatus() ); ?>"></div></td>
					<?php endif; ?>
					<?php if( in_array( 'catalog.lists.typeid', $fields ) ) : ?>
						<td class="catalog-lists-typeid"><?= $enc->html( $listItem->getType() ); ?></td>
					<?php endif; ?>
					<?php if( in_array( 'catalog.lists.config', $fields ) ) : ?>
						<td class="catalog-lists-config"><?= $enc->html( json_encode( $listItem->getConfig() ) ); ?></td>
					<?php endif; ?>
					<?php if( in_array( 'catalog.lists.datestart', $fields ) ) : ?>
						<td class="catalog-lists-datestart"><?= $enc->html( $listItem->getDateStart() ); ?></td>
					<?php endif; ?>
					<?php if( in_array( 'catalog.lists.dateend', $fields ) ) : ?>
						<td class="catalog-lists-dateend"><?= $enc->html( $listItem->getDateEnd() ); ?></td>
					<?php endif; ?>
					...
					<td class="actions">
						<a class="btn act-delete fa"
							href="<?= $enc->attr( $this->url( $delTarget, $delCntl, $delAction, ['resource' => 'catalog/lists', 'id' => $listItem->getId()] + $params, [], $delConfig ) ); ?>"
