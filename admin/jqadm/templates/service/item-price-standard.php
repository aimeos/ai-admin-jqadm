			...
			<div is="draggable" v-model="items" group="price" handle=".act-move">
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-price-group-item-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						v-bind:data-bs-target="'#item-price-group-data-' + idx" data-bs-toggle="collapse" role="tab" class="card-header header"
					...
					<div v-bind:id="'item-price-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div v-show="item['_ext']" class="col-xl-6 secondary" v-bind:class="{readonly: item['service.lists.siteid'] != siteid}">
							<config-table v-bind:tabindex="<?= $this->get( 'tabindex' ) ?>"
								...
								v-bind:name="'<?= $enc->attr( $this->formparam( ['price', '_idx_', 'config', '_pos_', '_key_'] ) ) ?>'.replace('_idx_', idx)"
