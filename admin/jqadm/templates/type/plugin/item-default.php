...
<form class="item item-plugin form-horizontal" method="POST" enctype="multipart/form-data" action="<?= $enc->attr( $this->url( $target, $cntl, $action, $params, [], $config ) ); ?>">
	...
	<nav class="main-navbar">
		<span class="navbar-brand">
			<?= $enc->html( $this->translate( 'admin', 'Plugin Type' ) ); ?>:
			<?= $enc->html( $this->get( 'itemData/plugin.type.id' ) ); ?> -
			<?= $enc->html( $this->get( 'itemData/plugin.type.label', $this->translate( 'admin', 'New' ) ) ); ?>
			<span class="navbar-secondary">(<?= $enc->html( $this->site()->match( $this->get( 'itemData/plugin.type.siteid' ) ) ); ?>)</span>
		</span>
	...
	<div class="row item-container">
		...
		<div class="col-md-9 item-content tab-content">
			...
			<div id="basic" class="row item-basic tab-pane fade show active" role="tabpanel" aria-labelledby="basic">
				...
				<div class="col-xl-6 content-block <?= $this->site()->readonly( $this->get( 'itemData/plugin.type.siteid' ) ); ?>">
					<div class="form-group row mandatory">
						...
						<div class="col-sm-8">
							<select class="form-control custom-select item-domain" required="required" tabindex="1"
								...
								<?php foreach( ['plugin', 'catalog', 'customer', 'media', 'price', 'product', 'service', 'supplier', 'text'] as $domain ) : ?>
									<option value="<?= $enc->attr( $domain ); ?>" <?= $selected( $this->get( 'itemData/plugin.type.domain', 'product' ), $domain ); ?> >
					...
					<div class="form-group row mandatory">
						...
						<div class="col-sm-12 form-text text-muted help-text">
							<?= $enc->html( $this->translate( 'admin', 'Unique type code, e.g. "color" for plugins like "green", "blue", "red"' ) ); ?>
						</div>
					</div>
					<div class="form-group row mandatory">
