...
<form class="item item-customer-property-type form-horizontal" method="POST" enctype="multipart/form-data" action="<?= $enc->attr( $this->url( $target, $cntl, $action, $params, [], $config ) ); ?>">
	...
	<nav class="main-navbar">
		<span class="navbar-brand">
			<?= $enc->html( $this->translate( 'admin', 'Customer Property Type' ) ); ?>:
			<?= $enc->html( $this->get( 'itemData/customer.property.type.id' ) ); ?> -
			<?= $enc->html( $this->get( 'itemData/customer.property.type.label', $this->translate( 'admin', 'New' ) ) ); ?>
			<span class="navbar-secondary">(<?= $enc->html( $this->site()->match( $this->get( 'itemData/customer.property.type.siteid' ) ) ); ?>)</span>
		</span>
	...
	<div class="row item-container">
		...
		<div class="col-md-9 item-content tab-content">
			...
			<div id="basic" class="row item-basic tab-pane fade show active" role="tabpanel" aria-labelledby="basic">
				...
				<div class="col-xl-6 content-block <?= $this->site()->readonly( $this->get( 'itemData/customer.property.type.siteid' ) ); ?>">
					<div class="form-group row mandatory">
						...
						<div class="col-sm-8">
							<select class="form-control custom-select item-domain" required="required" tabindex="1"
								...
								<?php foreach( ['customer', 'catalog', 'customer', 'media', 'price', 'product', 'service', 'supplier', 'text'] as $domain ) : ?>
									<option value="<?= $enc->attr( $domain ); ?>" <?= $selected( $this->get( 'itemData/customer.property.type.domain', 'product' ), $domain ); ?> >
