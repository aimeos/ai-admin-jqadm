...
<div id="image" class="item-image tab-pane fade" role="tabpanel" aria-labelledby="image">
	<div id="item-image-group" role="tablist" aria-multiselectable="true">
		...
		<?php foreach( $this->get( 'imageData/product.lists.id', [] ) as $idx => $id ) : ?>
			...
			<div class="group-item card <?= $this->site()->readonly( $this->get( 'imageData/product.lists.siteid/' . $idx ) ); ?>">
					...
					<div class="col-xl-6">
						<div class="form-group row image-preview">
							<input class="item-refid" type="hidden"
								name="<?= $enc->attr( $this->formparam( array( 'image', 'product.lists.refid', '' ) ) ); ?>"
								value="<?= $enc->attr( $this->get( 'imageData/product.lists.refid/' . $idx ) ); ?>" />
