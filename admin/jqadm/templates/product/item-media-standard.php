...
$keys = [
	'product.lists.id', 'product.lists.siteid', 'product.lists.type', 'product.lists.datestart', 'product.lists.dateend',
				...
				<div v-for="(entry, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-media-group-data-' + idx" v-bind:class="getCss(idx)"
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" class="form-control custom-select listitem-type" required
											...
											v-bind:name="'<?= $enc->attr( $this->formparam( ['media', 'idx', 'product.lists.type'] ) ); ?>'.replace('idx', idx)"
											v-bind:text="'<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>'"
											v-bind:readonly="checkSite('product.lists.siteid', idx)"
											v-bind:tabindex="'<?= $this->get( 'tabindex' ); ?>'"
											v-bind:value="entry['product.lists.type'] || 'default'" >
										</select>
									</div>
									<div class="col-sm-12 form-text text-muted help-text">
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'media', 'idx', 'product.lists.datestart' ) ) ); ?>'.replace( 'idx', idx )"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'media', 'idx', 'product.lists.dateend' ) ) ); ?>'.replace( 'idx', idx )"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
										v-bind:readonly="checkSite('product.lists.siteid', idx)"
						...
						<div v-show="advanced[idx]" class="col-xl-6 content-block secondary" v-bind:class="checkSite('product.lists.siteid', idx) ? 'readonly' : ''">
							<config-table inline-template v-bind:idx="idx" v-bind:items="getConfig(idx)" v-bind:readonly="checkSite('product.lists.siteid', idx)">
