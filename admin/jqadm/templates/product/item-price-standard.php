...
$keys = [
	'product.lists.id', 'product.lists.siteid', 'product.lists.typeid', 'product.lists.datestart', 'product.lists.dateend',
...
<div id="price" class="item-price content-block tab-pane fade" role="tablist" aria-labelledby="price">
	<div id="item-price-group" role="tablist" aria-multiselectable="true"
		...
		<div v-for="(id, idx) in items['product.lists.id']" class="group-item card">
			...
			<div v-bind:id="'item-price-group-data-' + idx" v-bind:class="getCss(idx)"
						...
						<div class="form-group row mandatory">
							...
							<div class="col-sm-8">
								<select class="form-control custom-select listitem-typeid" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
									name="<?= $enc->attr( $this->formparam( array( 'price', 'product.lists.typeid', '' ) ) ); ?>"
									v-bind:readonly="checkSite('product.lists.siteid', idx)"
									v-model="items['product.lists.typeid'][idx]" >

									<?php foreach( $this->get( 'priceListTypes', [] ) as $id => $typeItem ) : ?>
										<option value="<?= $enc->attr( $id ); ?>" v-bind:selected="items['product.lists.typeid'][idx] == '<?= $enc->attr( $id ) ?>'" >
											<?= $enc->html( $typeItem->getLabel() ); ?>
										</option>
									<?php endforeach; ?>
								</select>
					...
					<div class="form-group row optional">
						...
						<div class="col-sm-8">
							<input class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
								name="<?= $enc->attr( $this->formparam( array( 'price', 'product.lists.datestart', '' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:readonly="checkSite('product.lists.siteid', idx)"
								v-model="items['product.lists.datestart'][idx]" />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
					...
					<div class="form-group row optional">
						...
						<div class="col-sm-8">
							<input class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
								name="<?= $enc->attr( $this->formparam( array( 'price', 'product.lists.dateend', '' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:readonly="checkSite('product.lists.siteid', idx)"
								v-model="items['product.lists.dateend'][idx]" />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
				...
				<div v-show="advanced[idx]" class="col-xl-6 content-block secondary" v-bind:class="items['product.lists.siteid'][idx] != '<?= $this->site()->siteid() ?>' ? 'readonly' : ''">
