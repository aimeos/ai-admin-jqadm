						...
						<div class="form-group row mandatory">
							...
							<div class="col-sm-8">
								<select class="form-control custom-select listitem-typeid" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
									v-bind:name="'<?= $enc->attr( $this->formparam( array( 'image', 'idx', 'attribute.lists.typeid' ) ) ); ?>'.replace( 'idx', idx )"
									...
									<?php foreach( $this->get( 'imageListTypes', [] ) as $id => $typeItem ) : ?>
										<option value="<?= $enc->attr( $id ); ?>" v-bind:selected="items[idx]['attribute.lists.typeid'] == '<?= $enc->attr( $id ) ?>'" >
											<?= $enc->html( $typeItem->getLabel() ); ?>
										</option>
									<?php endforeach; ?>
								</select>
