				...
				<div v-for="(entry, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-media-group-item-' + idx" v-bind:class="getCss(idx)"
						...
						<div class="card-tools-right">
							<div v-if="!checkSite('supplier.lists.siteid', idx) && entry['supplier.lists.id'] != ''"
					...
					<div v-bind:id="'item-media-group-data-' + idx" v-bind:class="getCss(idx)"
						...
						<div class="col-xl-6">
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<select is="select-component" class="form-control custom-select item-languagid" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ); ?>"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'media', 'idx', 'supplier.lists.datestart' ) ) ); ?>'.replace( 'idx', idx )"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'media', 'idx', 'supplier.lists.dateend' ) ) ); ?>'.replace( 'idx', idx )"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
										v-bind:readonly="checkSite('supplier.lists.siteid', idx)"
						...
						<div v-show="advanced[idx]" class="col-xl-6 content-block secondary" v-bind:class="checkSite('supplier.lists.siteid', idx) ? 'readonly' : ''">
							<config-table inline-template
								v-bind:index="idx" v-bind:readonly="entry['supplier.lists.siteid'] != siteid"
