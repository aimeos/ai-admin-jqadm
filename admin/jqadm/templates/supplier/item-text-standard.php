...
$keys = [
	'supplier.lists.siteid', 'supplier.lists.typeid', 'supplier.lists.datestart', 'supplier.lists.dateend', 'config',
						...
						<div class="form-group row mandatory">
							...
							<div class="col-sm-8">
								<select class="form-control custom-select listitem-typeid" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
									v-bind:name="'<?= $enc->attr( $this->formparam( array( 'text', 'idx', 'supplier.lists.typeid' ) ) ); ?>'.replace('idx', idx)"
									...
									<?php foreach( $this->get( 'textListTypes', [] ) as $id => $typeItem ) : ?>
										<option value="<?= $enc->attr( $id ); ?>" v-bind:selected="entry['supplier.lists.typeid'] == '<?= $enc->attr( $id ) ?>'" >
											<?= $enc->html( $typeItem->getLabel() ); ?>
										</option>
									<?php endforeach; ?>
								</select>
