...
$keys = [
	'supplier.lists.siteid', 'supplier.lists.type', 'supplier.lists.datestart', 'supplier.lists.dateend', 'config',
				...
				<div v-for="(entry, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-text-group-item-' + idx" v-bind:class="getCss(idx)"
						...
						<div class="card-tools-right">
							<div v-if="!checkSite('supplier.lists.siteid', idx)" class="dropdown">
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" class="form-control custom-select item-type"
											...
											v-bind:tabindex="'<?= $this->get( 'tabindex' ); ?>'"
											v-bind:readonly="checkSite('text.siteid', idx)"
											v-bind:value="entry['text.languageid']" >
										</select>
									</div>
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" class="form-control custom-select listitem-type" required
											...
											v-bind:name="'<?= $enc->attr( $this->formparam( ['text', 'idx', 'supplier.lists.type'] ) ); ?>'.replace('idx', idx)"
											v-bind:text="'<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>'"
										...
										<tr v-for="(entry, pos) in list" v-bind:key="pos" class="config-item">
											<td class="config-row-key">
												<input is="auto-complete" class="form-control" v-bind:tabindex="1" v-bind:readonly="readonly" required
													v-bind:name="'<?= $enc->attr( $this->formparam( ['text', '_idx_', 'config', '_pos_', 'key'] ) ); ?>'.replace('_idx_', idx).replace('_pos_', pos)"
													v-bind:keys="JSON.parse('<?= $enc->attr( $this->config( 'admin/jqadm/supplier/item/text/config/suggest', [] ) ) ?>')"
													v-bind:value="entry.key" />
											</td>
											<td class="config-row-value">
												<input class="form-control" v-bind:tabindex="1" v-bind:readonly="readonly"
													v-bind:name="'<?= $enc->attr( $this->formparam( ['text', '_idx_', 'config', '_pos_', 'val'] ) ); ?>'.replace('_idx_', idx).replace('_pos_', pos)"
