			...
			<div is="draggable" v-model="items" group="text" handle=".act-move">
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-text-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" required class="form-control custom-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ); ?>"
											...
											v-bind:name="'<?= $enc->attr( $this->formparam( ['text', '_idx_', 'supplier.lists.type'] ) ); ?>'.replace('_idx_', idx)"
											v-bind:text="'<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>'"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'text', '_idx_', 'supplier.lists.datestart' ) ) ); ?>'.replace('_idx_', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'text', '_idx_', 'supplier.lists.dateend' ) ) ); ?>'.replace('_idx_', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
										v-bind:readonly="item['supplier.lists.siteid'] != siteid"
						...
						<div v-show="item['_ext']" class="col-xl-6 content-block secondary" v-bind:class="{readonly: item['supplier.lists.siteid'] != siteid}">
							<config-table inline-template
								v-bind:readonly="item['supplier.lists.siteid'] != siteid" v-bind:index="idx"
