...
<div id="image" class="item-image content-block tab-pane fade" role="tablist" aria-labelledby="image">
	<div id="item-image-group" role="tablist" aria-multiselectable="true"
		...
		<div v-for="(id, idx) in items['supplier.lists.id']" class="group-item card">
			<input class="item-listid" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'image', 'supplier.lists.id', '' ) ) ); ?>"
				v-bind:value="items['supplier.lists.id'][idx]" />
			...
			<div v-bind:id="'item-image-group-data-' + idx" v-bind:class="getCss(idx)"
						...
						<div class="form-group row mandatory">
							...
							<div class="col-sm-8">
								<select class="form-control custom-select listitem-typeid" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
									name="<?= $enc->attr( $this->formparam( array( 'image', 'supplier.lists.typeid', '' ) ) ); ?>"
									v-bind:readonly="checkSite('supplier.lists.siteid', idx)"
									v-model="items['supplier.lists.typeid'][idx]" >

									<?php foreach( $this->get( 'imageListTypes', [] ) as $id => $typeItem ) : ?>
										<option value="<?= $enc->attr( $id ); ?>" v-bind:selected="items['supplier.lists.typeid'][idx] == '<?= $enc->attr( $id ) ?>'" >
											<?= $enc->html( $typeItem->getLabel() ); ?>
										</option>
									<?php endforeach; ?>
								</select>
					...
					<div class="form-group row optional">
						...
						<div class="col-sm-8">
							<input class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
								name="<?= $enc->attr( $this->formparam( array( 'image', 'supplier.lists.datestart', '' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:readonly="checkSite('supplier.lists.siteid', idx)"
								v-model="items['supplier.lists.datestart'][idx]" />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
					...
					<div class="form-group row optional">
						...
						<div class="col-sm-8">
							<input class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
								name="<?= $enc->attr( $this->formparam( array( 'image', 'supplier.lists.dateend', '' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
								v-bind:readonly="checkSite('supplier.lists.siteid', idx)"
								v-model="items['supplier.lists.dateend'][idx]" />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">
				...
				<div v-show="advanced[idx]" class="col-xl-6 content-block secondary" v-bind:class="items['supplier.lists.siteid'][idx] != '<?= $this->site()->siteid() ?>' ? 'readonly' : ''">
