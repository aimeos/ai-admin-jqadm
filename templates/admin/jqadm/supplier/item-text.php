...
/** admin/jqadm/supplier/item/text/config/suggest
...
<div id="text" class="item-text tab-pane fade" role="tablist" aria-labelledby="text">
	...
	<div id="item-text-group"
		...
		<div class="group-list" role="tablist" aria-multiselectable="true">
			<div is="vue:draggable" v-model="items" group="text" handle=".act-move">
					...
					<div v-bind:id="'item-text-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div class="col-xl-6">
							...
							<div class="form-group row mandatory">
								<div class="col-sm-12">
									<textarea is="vue:html-editor" v-if="item['_show']" class="form-control item-content" required="required"
						...
						<div class="col-12 secondary item-meta text-muted">
							<small>
								...
								<span class="meta-value">{{ item['text.siteid'] }}</span>
							</small>
							<small>
								<?= $enc->html( $this->translate( 'admin', 'Editor' ) ) ?>:
								<span class="meta-value">{{ item['text.editor'] }}</span>
							</small>
							<small>
								<?= $enc->html( $this->translate( 'admin', 'Created' ) ) ?>:
								<span class="meta-value">{{ item['text.ctime'] }}</span>
							</small>
							<small>
								<?= $enc->html( $this->translate( 'admin', 'Modified' ) ) ?>:
								<span class="meta-value">{{ item['text.mtime'] }}</span>
							</small>
						</div>
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="vue:select-component" required class="form-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
											...
											v-bind:name="`<?= $enc->js( $this->formparam( ['text', '_idx_', 'supplier.lists.type'] ) ) ?>`.replace('_idx_', idx)"
											v-bind:text="`<?= $enc->js( $this->translate( 'admin', 'Please select' ) ) ?>`"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="vue:flat-pickr" class="form-control listitem-datestart select" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'text', '_idx_', 'supplier.lists.datestart' ) ) ) ?>`.replace('_idx_', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input is="vue:flat-pickr" class="form-control listitem-dateend select" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'text', '_idx_', 'supplier.lists.dateend' ) ) ) ?>`.replace('_idx_', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
										v-bind:disabled="item['supplier.lists.siteid'] != siteid"
										v-bind:config="Aimeos.flatpickr.datetime"
						...
						<div v-show="item['_ext']" class="col-12 secondary item-meta text-muted">
							<small>
								<?= $enc->html( $this->translate( 'admin', 'Site' ) ) ?>:
								<span class="meta-value">{{ item['supplier.lists.siteid'] }}</span>
							</small>
							<small>
								<?= $enc->html( $this->translate( 'admin', 'Editor' ) ) ?>:
								<span class="meta-value">{{ item['supplier.lists.editor'] }}</span>
							</small>
							<small>
								<?= $enc->html( $this->translate( 'admin', 'Created' ) ) ?>:
								<span class="meta-value">{{ item['supplier.lists.ctime'] }}</span>
							</small>
							<small>
								<?= $enc->html( $this->translate( 'admin', 'Modified' ) ) ?>:
								<span class="meta-value">{{ item['supplier.lists.mtime'] }}</span>
							</small>
						</div>
