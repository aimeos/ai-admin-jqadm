						...
						<div v-bind:id="'item-text-group-data-' + index" v-bind:class="element['_show'] ? 'show' : 'collapsed'"
							...
							<div v-show="element['_ext']" class="col-xl-6 secondary">
									...
									<div class="form-group row mandatory">
										...
										<div class="col-sm-8">
											<select is="vue:select-component" required class="form-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ) ?>"
												...
												v-bind:text="`<?= $enc->js( $this->translate( 'admin', 'Please select' ) ) ?>`"
												v-bind:readonly="element['supplier.lists.siteid'] != siteid"
												v-model="element['supplier.lists.type']" >
											</select>
										</div>
										<div class="col-sm-12 form-text text-muted help-text">
								...
								<?php else : ?>
									<input class="listitem-type" type="hidden"
										v-bind:name="`<?= $enc->js( $this->formparam( array( 'text', '_idx_', 'supplier.lists.type' ) ) ) ?>`.replace('_idx_', index)"
								...
								<div class="form-group row optional">
									...
									<div class="col-sm-8">
										<input is="vue:flat-pickr" class="form-control listitem-datestart select" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'text', '_idx_', 'supplier.lists.datestart' ) ) ) ?>`.replace('_idx_', index)"
											placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
											v-bind:disabled="element['supplier.lists.siteid'] != siteid"
											v-bind:config="Aimeos.flatpickr.datetime"
											v-model="element['supplier.lists.datestart']">
									</div>
									<div class="col-sm-12 form-text text-muted help-text">
								...
								<div class="form-group row optional">
									...
									<div class="col-sm-8">
										<input is="vue:flat-pickr" class="form-control listitem-dateend select" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ) ?>"
											v-bind:name="`<?= $enc->js( $this->formparam( array( 'text', '_idx_', 'supplier.lists.dateend' ) ) ) ?>`.replace('_idx_', index)"
											placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ) ?>"
											v-bind:disabled="element['supplier.lists.siteid'] != siteid"
											v-bind:config="Aimeos.flatpickr.datetime"
											v-model="element['supplier.lists.dateend']">
									</div>
									<div class="col-sm-12 form-text text-muted help-text">
							...
							<div v-show="element['_ext']" class="col-xl-6 secondary">
								<config-table v-bind:tabindex="`<?= $enc->js( $this->get( 'tabindex' ) ) ?>`"
									...
									v-bind:index="index" v-bind:readonly="element['supplier.lists.siteid'] != siteid"
							...
							<div v-show="element['_ext']" class="col-12 secondary item-meta text-muted">
								<small>
									<?= $enc->html( $this->translate( 'admin', 'Site' ) ) ?>:
									<span class="meta-value">{{ element['supplier.lists.siteid'] }}</span>
								</small>
								<small>
									<?= $enc->html( $this->translate( 'admin', 'Editor' ) ) ?>:
									<span class="meta-value">{{ element['supplier.lists.editor'] }}</span>
								</small>
								<small>
									<?= $enc->html( $this->translate( 'admin', 'Created' ) ) ?>:
									<span class="meta-value">{{ element['supplier.lists.ctime'] }}</span>
								</small>
								<small>
									<?= $enc->html( $this->translate( 'admin', 'Modified' ) ) ?>:
									<span class="meta-value">{{ element['supplier.lists.mtime'] }}</span>
								</small>
							</div>
