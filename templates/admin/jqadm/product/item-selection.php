				...
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-selection-group-data-' + idx" v-bind:class="css(idx)"
						...
						<input type="hidden" v-model="item['quantity']"
							v-bind:name="`<?= $enc->js( $this->formparam( ['selection', '_idx_', 'quantity'] ) ) ?>`.replace('_idx_', idx)">
									...
									<tr v-for="(attr, attridx) in (item['attr'] || [])" :key="attridx">
										<td>
											...
											<input class="item-attr-refid" type="hidden" v-model="attr['attribute.id']"
												v-bind:name="`<?= $enc->js( $this->formparam( ['selection', '_idx_', 'attr', '_attridx_', 'attribute.id'] ) ) ?>`.replace('_idx_', idx).replace( '_attridx_', attridx)">

											<input class="item-attr-label" type="hidden" v-model="attr['attribute.label']"
												v-bind:name="`<?= $enc->js( $this->formparam( ['selection', '_idx_', 'attr', '_attridx_', 'attribute.label'] ) ) ?>`.replace('_idx_', idx).replace( '_attridx_', attridx)">
											...
											<Multiselect class="item-id form-control"
												...
												:disabled="!can('change', idx, attridx)"
