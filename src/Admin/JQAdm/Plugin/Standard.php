	...
	implements \Aimeos\Admin\JQAdm\Common\Admin\Factory\Iface
{
	...
	public function save() : ?string
	{
		...
		try
		{
			...
			// @phpstan-ignore argument.type
			return $this->redirect( 'plugin', (string) $view->param( 'next' ), $view->item->getId(), 'save' );
		}
		catch( \Exception $e )
		{
			$manager->rollback();
	...
	public function search() : ?string
	{
		...
		{
			$total = 0;
			$params = $this->storeFilter( (array) $view->param(), 'plugin' );
			$manager = \Aimeos\MShop::create( $this->context(), 'plugin' );
	...
	protected function getSubClientNames() : array
	{
		 ...
		 * @since 2017.10
		 */
		return (array) $this->context()->config()->get( 'admin/jqadm/plugin/subparts', [] );
	}
	...
	protected function fromArray( array $data ) : \Aimeos\MShop\Plugin\Item\Iface
	{
		...
		if( isset( $data['plugin.id'] ) && $data['plugin.id'] != '' ) {
			$item = $manager->get( (string) $data['plugin.id'] );
		} else {
			$item = $manager->create();
		}
